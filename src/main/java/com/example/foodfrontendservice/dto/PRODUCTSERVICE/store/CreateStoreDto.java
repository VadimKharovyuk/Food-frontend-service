...
import com.example.foodfrontendservice.dto.PRODUCTSERVICE.store.CreateAddressDto;
...
public class CreateStoreDto {
    ...
    // Метод для создания объекта адреса (если нужен для сервиса)
    public CreateAddressDto getAddressObject() {
    ...
    public String getImageFileName() {
        return hasImageFile() ? imageFile.getOriginalFilename() : null;
    }

    public long getImageFileSize() {
        return hasImageFile() ? imageFile.getSize() : 0;
    }

    public String getImageContentType() {
        return hasImageFile() ? imageFile.getContentType() : null;
    }
    ...
    public boolean isValid() {
        return name != null && !name.trim().isEmpty() &&
                ...
                estimatedDeliveryTime != null && estimatedDeliveryTime > 0;
    }

    @Override
    public String toString() {
        return String.format("CreateStoreDto{name='%s', city='%s', deliveryRadius=%d, hasImage=%s}",
                name, city, deliveryRadius, hasImageFile());
    }
}